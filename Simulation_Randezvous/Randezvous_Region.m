function Randezvous_Region(cfg)
    ...
    for s_idx = 1:length(sigma_cases_deg)
        ...
        % 1. 시뮬레이션 결과 점 찍기 (DisplayName 추가!)
        if ~isempty(Fail_X)
            plot(ax, Fail_X, Fail_Y, '.', 'MarkerSize', 5, 'Color', [1 0.4 0.4], 'DisplayName', 'Fail'); 
        end
        if ~isempty(Suc_X)
            plot(ax, Suc_X, Suc_Y, '.', 'MarkerSize', 6, 'Color', 'b', 'DisplayName', 'Success'); 
        end
        
        % 타겟 위치도 범례에 넣어주면 보기 좋겠지?
        plot(ax, 0, 0, '^', 'MarkerSize', 10, 'MarkerFaceColor', 'g', 'Color', 'g', 'DisplayName', 'Target');
        ...
        % 범례 표시 (위치가 겹치지 않게 알아서 비켜주도록 'best' 설정)
