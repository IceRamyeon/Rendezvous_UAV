...
% 범례(Legend) 달기 (추가된 요소들 반영!)
h_red   = plot(nan, nan, 'r', 'LineWidth', 1.5);
h_purp  = plot(nan, nan, 'm', 'LineWidth', 1.5);
h_safe  = patch(nan, nan, [0.7 0.8 1.0], 'EdgeColor', 'none'); 
h_reach = patch(nan, nan, [0.8 0.7 0], 'EdgeColor', 'none'); % DPG Reachable Region (노란색/주황색 계열 가정)
h_inter = patch(nan, nan, [0 0.8 0], 'EdgeColor', 'none'); 
h_tangent = plot(nan, nan, 'rp', 'MarkerSize', 10, 'MarkerFaceColor', 'r');

legend([h_red, h_purp, h_safe, h_reach, h_inter, h_tangent], ...
    {'+g Limit (Unsafe)', '-g Limit (Unsafe)', 'Safe Region', 'DPG Reachable Region', 'Intersect Region', 'Tangent Point'}, ...
