function Trace_Max_Acc(varargin)
    ...
    % Trace_Max_Acc(sigma_p_deg, sigma_ref_deg, V_p, V_t, gain_k, limit_acc)
    ...
    % --- [+Acc Limit] 영역 (Cyan) ---
    if any(r_pos > 0)
        fill(ax, X_pos, Y_pos, 'c', 'FaceAlpha', 0.15, 'EdgeColor', 'none', ...
            'Tag', 'PosAccFill', 'HandleVisibility', 'off');
        plot(ax, X_pos, Y_pos, 'c-.', 'LineWidth', 1.5, ...
            'DisplayName', '+Acc Limit', 'Tag', 'PosAccLine');
    end
    
    % --- [-Acc Limit] 영역 (Magenta) ---
    if any(r_neg > 0)
        fill(ax, X_neg, Y_neg, 'm', 'FaceAlpha', 0.15, 'EdgeColor', 'none', ...
            'Tag', 'NegAccFill', 'HandleVisibility', 'off');
        plot(ax, X_neg, Y_neg, 'm-.', 'LineWidth', 1.5, ...
