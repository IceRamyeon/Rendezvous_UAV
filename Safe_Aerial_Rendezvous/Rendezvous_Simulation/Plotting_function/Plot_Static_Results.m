function Plot_Static_Results(cfg, sim_out)
    ...
    if strcmp(cfg.GUIDANCE_MODE, 'RDPG_FRS') && isfield(sim_out, 'point_value_hist')
        ...
        hold on; grid on;
        title('[Fig 8] Selected FRS Distance History');
        xlabel('Time (s)');
        ylabel('Distance to Boundary');
        
        num_steps = length(time);
        selected_dist = nan(1, num_steps);
        
        % 스텝별로 채택된(2열이 1인) 각도의 거리(1열) 추출
        for i = 1:num_steps
            pts = sim_out.point_value_hist{i};
            if ~isempty(pts) && size(pts, 2) >= 2
                % 2열이 1인 인덱스 찾기 (여러 개면 첫 번째 것만)
                idx = find(pts(:, 2) == 1, 1);
                if ~isempty(idx)
                    selected_dist(i) = pts(idx, 1);
                end
            end
        end
        
        % 채택된 거리만 단일 빨간 선으로 플롯
        plot(time, selected_dist, 'r-', 'LineWidth', 2);
