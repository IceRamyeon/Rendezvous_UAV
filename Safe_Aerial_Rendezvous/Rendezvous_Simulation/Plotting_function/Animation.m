function Animation(cfg, sim_out)
    ...
    if cfg.auto_save == 1
        ...
        vidObj3 = VideoWriter(fullfile(cfg.save_dir, 'Figure_3.mp4'), 'MPEG-4'); 
        vidObj3.FrameRate = 12; 
        open(vidObj3);
        
        vidObj4 = VideoWriter(fullfile(cfg.save_dir, 'Figure_4.mp4'), 'MPEG-4'); 
        vidObj4.FrameRate = 12; 
        open(vidObj4);
        
        vidObj5 = VideoWriter(fullfile(cfg.save_dir, 'Figure_5.mp4'), 'MPEG-4'); 
        vidObj5.FrameRate = 12; 
        open(vidObj5);
    end
                    ...
                    if ~isempty(curr_pts) && size(curr_pts, 2) >= 4
                        for k = 1:length(h_frs_pts)
                            if k <= size(curr_pts, 1)
                                % RDPG_FRS에서 넘어온 좌표는 Inertial Frame에서의 상대 좌표이므로
                                % Fig 3(Target frame)에 맞게 회전 변환 적용
                                raw_x = curr_pts(k, 3);
                                raw_y = curr_pts(k, 4);
