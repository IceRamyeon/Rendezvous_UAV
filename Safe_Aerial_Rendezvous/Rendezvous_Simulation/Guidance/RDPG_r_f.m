classdef RDPG_r_f < handle
    ...
    methods
        function obj = RDPG_r_f(k_gain, limit_G, r_f_escape, r_f_max, dt, init_sigma_rad)
            obj.k = k_gain;
            obj.max_acc = limit_G * 9.81;
            obj.r_f_escape = r_f_escape;
            obj.r_f_max = r_f_max; 
            obj.dt = dt;
            ...
            if r >= r_contour_min && r <= r_contour_max
                ...
                try
                    ...
                    if r_f_compare > obj.r_f_max
                        ...
                        % [4단계] Fail-Safe 탈출
                        % -------------------------------------------------------
                        % r_f_escape을 사용하여 큰 회전 반경 확보. Pursuer가 Reachable Region 내에 들어왔을 때 
                        % 회선할 수 있는 여유가 생김. r_f_escape을 통해 sigma_pc2을 찾음.
                        
                        % x = sigma_pc2
                        eqn_escape = @(x) sqrt(r/obj.r_f_escape) * cos((sigma_t + x)/2) - cos(x);
                        try 
