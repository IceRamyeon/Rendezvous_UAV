...
% [유도 법칙 설정]
GUIDANCE_Switch = 1;    % 1 = PPNG, 2 = DPG, 3 = VDPG, 4 = RDPG, 5 = RDPG_T, 6 = RDPG_SAFE
...
cfg.tf = 40;                    % Final Time
cfg.pause_t = 0.1;              % Pause Time
cfg.skip_frame = 1;            % Animation 속도 조절, auto_save가 켜지면 느려지므로 Frame을 70 정도로 잡을 것.

% Stop Condition / auto save 사용여부
cfg.stop_condition = 1;         % 0 = off, 1 = on, 단 RDPG_T는 무조건 on
cfg.auto_save = 0;
target_path = 'C:\Users\최혁재\Desktop\대학자료\AISL 연구실\260303 랩미팅 준비'; 
cfg.save_dir = fullfile(target_path, 'bearing = 30도,RDPG');
...
switch RUN_MODE
    case 'REG'
        % --- Region 모드 전용 설정 ---
        % (거리 Sweep)
        cfg.r_min_km = 0.1; cfg.r_max_km = 1; cfg.r_step_km = 0.01;
        
        % (Bearing Sweep)
        cfg.b_min_deg = -89; cfg.b_max_deg = -1; cfg.b_step_deg = 1.0;

        % (Sigma Case)
        cfg.sigma_cases_deg = [60, 65, 70, 75, 80, 85];
    ...
    case 'SIM'
        ...
        cfg.psi_ti_deg = 120;        % [deg] 타겟 초기 헤딩

        % [Pursuer 초기 상대 위치 (Region 분석 결과값 대입)]
        cfg.r_from_region_m = 1000;      % [m] 초기 거리
        cfg.bearing_from_region = -50;    % [deg] 초기 베어링 (Target 기준)
        lead_p_init = 0;
